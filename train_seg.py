...
def get_dl(test=False):
    from dataset_prepare.dataset import Dataset      # change dataset
...
# net
def get_model():
    from net.Unet_2D import Unet        # change model
...
def train():
    ...
    for epoch in range(1, Epoch+1):
        ...
        # save
        if epoch % 5 == 0:
            torch.save(model.state_dict(), './saved_models/seg/net{}-{:.3f}.pth'.format(epoch, train_loss))
...
def save_loss(losses):
    ...
    df.to_csv('./out/loss/seg:%s.csv' % datetime.datetime.now())
